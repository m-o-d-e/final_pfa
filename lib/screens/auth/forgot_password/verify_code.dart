...
class ForgotPasswordVerifyCodePage extends StatefulWidget {
  ...
  final String countryCode;
   ...
   ForgotPasswordVerifyCodePage({Key? key, required  this.phoneNumber, required this.countryCode}) : super(key: key){
    print('ForgotPasswordVerifyCodePage');
  }
  @override
  _ForgotPasswordVerifyCodePage createState() => _ForgotPasswordVerifyCodePage();
}

class _ForgotPasswordVerifyCodePage extends State<ForgotPasswordVerifyCodePage> {
  String otp = '';
  int _counter = 60;
  Timer? _timer;
          ...
          return BlocProvider<ForgotPasswordBloc>(
              ...
              child: BlocConsumer<ForgotPasswordBloc, ForgotPasswordState>(
                  listener: (context, state) {
                    if(state is ForgotPasswordLoadingState) {
                    ...
                    else if(state is ForgotPasswordResendLoadingState){
                      QuickAlert.show(
                        context: context,
                        type: QuickAlertType.loading,
                    ...
                    else if(state is ForgotPasswordVerifyCodeErrorState) {
                      if (context.mounted) {
                    ...
                    else if (state is ForgotPasswordVerifyCodeSuccessState) {
                      if (context.mounted) {
                      ...
                      Navigator.push(
                        context,
                        MaterialPageRoute(builder: (context) => NewPasswordPage(phoneNumber: widget.phoneNumber, countryCode: widget.countryCode)),
                      );
                    }
                    else if(state is ForgotPasswordResendErrorState){
                      if (context.mounted) {
                        Navigator.of(context, rootNavigator: true)
                            .pop(); // dismiss the loading dialog
                        QuickAlert.show(
                    ...
                    else if(state is ForgotPasswordResendSuccessState) {
                      if (context.mounted) {
                        Navigator.of(context, rootNavigator: true)
                            .pop(); // dismiss the loading dialog
                        QuickAlert.show(
  ...
  Future<bool> popScreen(BuildContext context, state) async {
    // disable back button on the device
    await showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Êtes-vous sûr d\'annuler la réinitialisation du mot de passe?'),
        actions: [
          TextButton(
